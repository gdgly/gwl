...
//int mrsFind645Frame(HI_IN HI_PBYTE in_buf, HI_IN HI_S16 in_len,
  //                          HI_OUT HI_U16 *start, HI_OUT HI_U16 *out_len)
int mrsFind645Frame(unsigned char* in_buf, int in_len,
                            int *start, int *out_len)
{
    int ret = -1;//HI_ERR_FAILURE;
    unsigned char* buf = in_buf;

    if((!in_buf) || (!start) || (!out_len))
    {
        return -1;//HI_ERR_INVALID_PARAMETER;
    }
    ...
    while (in_len > 0)
    {
        ...
        head = (unsigned char* )mrsToolsStreamFindByte(buf,in_len,0x68);
        ...
        {//找到帧
            ...
            if (frame_len > MRS_PROTO645_DATAGRAM_LEN_MAX)
            {
                buf++;
                in_len--;				
				printf("frame_len > MRS_PROTO645_DATAGRAM_LEN_MAX\n");
                continue;
            }

            if(in_len >= frame_len)
            {
                if(0 == mrsToolsCheck645Frame(buf,frame_len))
                {
                    *start = (int)(buf - in_buf);
                    *out_len = frame_len;

                    ret = 0;//HI_ERR_SUCCESS;
                    break;
                }
                else
                {
                    buf++;
                    in_len--;
					printf("mrsToolsCheck645Frame is not equal zero\n");
                    continue;
                }
            }
            else
